...
def read_from_csv(file_name):
    try:
        ...
        logger.error("Could not read file: I/O error({0}): {1}".format(e.errno, e.strerror))
    except Exception, ex:
        print(ex)
...
def main():
    ...
    result.verbose = True
    setup_logging(logfile="checkpoint_signature.log", scrnlog=result.verbose)
    logger.info(
    ...
    database_name = 'nfdb'
    data_path = os.path.join(root_path, "CheckPoint_Signature")
    if not os.path.exists(data_path):
        os.makedirs(data_path)
    file_path = os.path.join(data_path, "checkpoint_difference.txt")
    try:
        ...
        file_object.write("%s\n" % "alarm_id")
        for item in results:
            alarm_name = item['Protection'].strip()
            if alarm_name not in db_signature_details.keys():
                file_object.write('%s \n' % alarm_name)
        file_object.close()
